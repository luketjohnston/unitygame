...
{
    [BurstCompile]
    public struct CanMoveGhostComponentSerializer
    {
        static CanMoveGhostComponentSerializer()
        {
            State = new GhostComponentSerializer.State
            {
                GhostFieldsHash = 14767913548786401661,
                ExcludeFromComponentCollectionHash = 0,
                ComponentType = ComponentType.ReadWrite<CanMove>(),
                ComponentSize = UnsafeUtility.SizeOf<CanMove>(),
        ...
        private static void CopyToSnapshot(IntPtr stateData, IntPtr snapshotData, int snapshotOffset, int snapshotStride, IntPtr componentData, int componentStride, int count)
        {
            for (int i = 0; i < count; ++i)
            {
                ...
                ref var component = ref GhostComponentSerializer.TypeCast<CanMove>(componentData, componentStride*i);
                ref var serializerState = ref GhostComponentSerializer.TypeCast<GhostSerializerState>(stateData, 0);
                snapshot.Value = component.Value?1u:0;
            }
        }
        [BurstCompile]
        [MonoPInvokeCallback(typeof(GhostComponentSerializer.CopyToFromSnapshotDelegate))]
        ...
        {
            ref var component = ref GhostComponentSerializer.TypeCast<CanMove>(componentData, 0);
            ref var backup = ref GhostComponentSerializer.TypeCast<CanMove>(backupData, 0);
        ...
        private static void ReportPredictionErrors(IntPtr componentData, IntPtr backupData, ref UnsafeList<float> errors)
        {
            ...
            ref var backup = ref GhostComponentSerializer.TypeCast<CanMove>(backupData, 0);
            int errorIndex = 0;
            errors[errorIndex] = math.max(errors[errorIndex], (component.Value != backup.Value) ? 1 : 0);
            ++errorIndex;
        }
