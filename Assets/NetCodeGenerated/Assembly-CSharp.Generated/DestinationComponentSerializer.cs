...
{
    [BurstCompile]
    public struct DestinationComponentGhostComponentSerializer
    {
        static DestinationComponentGhostComponentSerializer()
        {
            State = new GhostComponentSerializer.State
            {
                GhostFieldsHash = 15282735658198342537,
                ExcludeFromComponentCollectionHash = 0,
                ComponentType = ComponentType.ReadWrite<DestinationComponent>(),
                ComponentSize = UnsafeUtility.SizeOf<DestinationComponent>(),
        ...
        private static void CopyToSnapshot(IntPtr stateData, IntPtr snapshotData, int snapshotOffset, int snapshotStride, IntPtr componentData, int componentStride, int count)
        {
            for (int i = 0; i < count; ++i)
            {
                ...
                ref var component = ref GhostComponentSerializer.TypeCast<DestinationComponent>(componentData, componentStride*i);
                ...
                snapshot.Valid = component.Valid?1u:0;
            }
        }
        [BurstCompile]
        [MonoPInvokeCallback(typeof(GhostComponentSerializer.CopyToFromSnapshotDelegate))]
        private static void CopyFromSnapshot(IntPtr stateData, IntPtr snapshotData, int snapshotOffset, int snapshotStride, IntPtr componentData, int componentStride, int count)
        {
            for (int i = 0; i < count; ++i)
            {
                ...
                component.Value = new float2(snapshotBefore.Value_x * 0.001f, snapshotBefore.Value_y * 0.001f);
                component.Valid = snapshotBefore.Valid != 0;
            }
        }
        [BurstCompile]
        ...
        {
            ref var component = ref GhostComponentSerializer.TypeCast<DestinationComponent>(componentData, 0);
            ref var backup = ref GhostComponentSerializer.TypeCast<DestinationComponent>(backupData, 0);
            component.Value.x = backup.Value.x;
            component.Value.y = backup.Value.y;
            component.Valid = backup.Valid;
        }
        ...
        private static void PredictDelta(IntPtr snapshotData, IntPtr baseline1Data, IntPtr baseline2Data, ref GhostDeltaPredictor predictor)
        {
            ...
            snapshot.Valid = (uint)predictor.PredictInt((int)snapshot.Valid, (int)baseline1.Valid, (int)baseline2.Valid);
        }
        [BurstCompile]
        ...
        private static void CalculateChangeMask(IntPtr snapshotData, IntPtr baselineData, IntPtr bits, int startOffset)
        {
            ...
            changeMask |= (snapshot.Valid != baseline.Valid) ? (1u<<1) : 0;
        ...
        private static void Serialize(IntPtr snapshotData, IntPtr baselineData, ref DataStreamWriter writer, ref NetworkCompressionModel compressionModel, IntPtr changeMaskData, int startOffset)
        {
            ...
            if ((changeMask & (1 << 1)) != 0)
                writer.WritePackedUIntDelta(snapshot.Valid, baseline.Valid, compressionModel);
        }
        [BurstCompile]
        ...
        private static void Deserialize(IntPtr snapshotData, IntPtr baselineData, ref DataStreamReader reader, ref NetworkCompressionModel compressionModel, IntPtr changeMaskData, int startOffset)
        {
            ...
            if ((changeMask & (1 << 1)) != 0)
                snapshot.Valid = reader.ReadPackedUIntDelta(baseline.Valid, compressionModel);
            else
                snapshot.Valid = baseline.Valid;
        }
        #if UNITY_EDITOR || DEVELOPMENT_BUILD
        [BurstCompile]
        ...
        private static void ReportPredictionErrors(IntPtr componentData, IntPtr backupData, ref UnsafeList<float> errors)
        {
            ...
            ++errorIndex;
            errors[errorIndex] = math.max(errors[errorIndex], (component.Valid != backup.Valid) ? 1 : 0);
            ++errorIndex;
        }
        private static int GetPredictionErrorNames(ref FixedString512 names)
        {
            ...
            if (nameCount != 0)
                names.Append(new FixedString32(","));
            names.Append(new FixedString64("Valid"));
            ++nameCount;
            return nameCount;
        }
        #endif
